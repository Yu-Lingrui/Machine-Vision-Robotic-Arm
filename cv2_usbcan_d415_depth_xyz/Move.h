...
void Rcv()
{
	VCI_CAN_OBJ frame[50];
	while (true)
	{
		int i;
		int len = 0;
		len = VCI_Receive(3, 0, 0, frame, 50, 200); 
		if (len <= 0)
		{
			std::cout << "读取失败" << std::endl;
		}										 
		else
		{
			...
			for (i = 0; i < len; i++)
			{
				unsigned char* pdata = &frame[i].Data[0];
				if ((frame[i].DataLen >= 5) && (pdata[0] == 'G') && (pdata[1] == 'S'))//正常模式
				{
					short temp_value = 0;
					int m_CurrPos;
					int NodeID = frame[i].ID - 0x20;
					temp_value = pdata[2] + (pdata[3] << 8);
					m_CurrPos = (int)temp_value / 64;
					if (NodeID ==8)
					{
